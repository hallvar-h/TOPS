...
class PowerSystemModel:
    ...
    def init_dyn_sim_old(self):
        ...
        def add_states(start_idx, data, states):
            ...
            mdl.states = np.recarray((len(data),), dtype=[(state, int) for state in states])
            for state in states:
                mdl.states[state] = mdl.idx[state_desc_mdl[:, 1] == state]
            mdl.par = data.to_records()
        ...
        ####################
        # Experimental: Gen in separate file
        key = 'GEN'
        self.gen_mdls = dict()
        ...
        start_idx = len(self.state_desc_2)
        ...
        # # Generators
        # self.x0_2 = self.x0.copy()
        # for key in self.gen_mdls.keys():
        #     dm = self.gen_mdls[key]
        #     x_0, outputs_0 = dm.initialize(self.v_g, self.s_g*self.s_n/self.P_n_gen)
        #     self.x0_2[dm.idx] = x_0
        #     e_q_0, P_m_0 = outputs_0
    ...
    def ode_fun_old(self, t, x):
        ...
        #     dx_loc, output = dm.update(self.f, x[dm.idx], self.v_g, inputs)
        #     dx_2[dm.idx] = dx_loc
    ...
    def ode_fun(self, t, x):
        ...
        # self.i_inj_d_2 = np.zeros(self.n_bus_red, dtype=complex)
        # self.i_inj_q_2 = np.zeros(self.n_bus_red, dtype=complex)
        for key in self.gen_mdls.keys():
            dm = self.gen_mdls[key]
            i_inj_d_mdl, i_inj_q_mdl = dm.current_injections(x[dm.idx])*self.I_n_gen/self.i_n[self.gen_bus_idx]
